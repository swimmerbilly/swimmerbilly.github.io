...
def permit_stats(db: Session = Depends(get_db)) -> PermitStats:
    ...
    for permit in permits:
        ...
        if permit.has_structural_plans:
            structural += 1
        if permit.structural_engineer_name:
            with_engineer += 1
    return PermitStats(
        total=len(permits),
...
def list_permits(
    ...
    if engineer:
        ...
        query = query.filter(BuildingPermit.structural_engineer_name.ilike(like))
    if q:
        like = f"%{q.strip()}%"
        query = query.filter(
...
async def sync_one_jurisdiction(
    jurisdiction_id: str,
    max_results: int = Query(default=40, ge=5, le=100),
    db: Session = Depends(get_db),
) -> PermitSyncRunRead:
    try:
        run = await sync_jurisdiction(db, jurisdiction_id, max_results=max_results)
...
async def sync_crawlable_jurisdictions(
    max_results: int = Query(default=30, ge=5, le=100),
    db: Session = Depends(get_db),
) -> list[PermitSyncRunRead]:
    return await sync_all_crawlable(db, max_results=max_results)
